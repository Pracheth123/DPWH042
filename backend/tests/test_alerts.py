...
def test_alerts_returns_non_neutral_signals():
    _seed("logistics", 0.8, severity="high")
    _seed("price", 0.6, severity="medium")
    response = client.get("/api/alerts")
    data = response.json()
    assert data["total"] == 2
...
def test_alerts_sorted_by_confidence_descending():
    _seed("logistics", 0.9, severity="high")
    _seed("price", 0.4, severity="medium")
    _seed("behavior", 0.7, severity="high")
...
def test_alerts_filter_by_signal_type():
    _seed("logistics", 0.8, severity="high")
    _seed("price", 0.6, severity="medium")
    response = client.get("/api/alerts?signal_type=logistics")
    data = response.json()
    assert all(a["signal_type"] == "logistics" for a in data["alerts"])


def test_alerts_filter_by_severity():
    _seed("logistics", 0.9, severity="high")
    _seed("price", 0.3, severity="low")
...
def test_alerts_filter_by_language():
    _seed("logistics", 0.8, language="ur", severity="high")
    _seed("price", 0.7, language="ar", severity="high")
...
def test_get_alert_by_id_found():
    record = _seed("logistics", 0.8, severity="high")
